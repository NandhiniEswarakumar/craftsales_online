...
const AdminPanel = () => {
  ...
  const loadDashboardData = async () => {
    setLoading(true);
    try {
  const res = await fetch(`${process.env.REACT_APP_API_BASE_URL || ''}/api/admin/dashboard-stats`, {
  ...
  const res = await fetch(`${process.env.REACT_APP_API_BASE_URL || ''}/api/admin/users?${params}`, {
        headers: { 'x-admin-password': adminPassword }
      });
  ...
  const loadLoginHistory = async () => {
    setLoading(true);
    try {
  const res = await fetch(`${process.env.REACT_APP_API_BASE_URL || ''}/api/admin/user-logins`, {
        headers: { 'x-admin-password': adminPassword }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const toggleAdminStatus = async (userId) => {
    try {
  const res = await fetch(`${process.env.REACT_APP_API_BASE_URL || ''}/api/admin/user/${userId}/toggle-admin`, {
        method: 'PATCH',
        headers: { 
  ...
  const verifyUserEmail = async (userId) => {
    try {
  const res = await fetch(`${process.env.REACT_APP_API_BASE_URL || ''}/api/admin/user/${userId}/verify-email`, {
        method: 'PATCH',
        headers: { 
          'x-admin-password': adminPassword,
          'Content-Type': 'application/json'
  ...
  const resendVerificationEmail = async (userId, userEmail) => {
    try {
  const res = await fetch(`${process.env.REACT_APP_API_BASE_URL || ''}/api/resend-verification`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json'
        },
  ...
  const deleteUser = async (userId) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      try {
  const res = await fetch(`${process.env.REACT_APP_API_BASE_URL || ''}/api/admin/user/${userId}`, {
          method: 'DELETE',
          headers: { 'x-admin-password': adminPassword }
        });
